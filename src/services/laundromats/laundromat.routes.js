...
router.post('/register', async (req, res) => {
  const { name, address, area, city, mpesa_till, description, owner_name, email, phone, password,
          payout_method, payout_number, payout_account_ref, payout_name,
          terms_accepted, terms_version } = req.body;
  ...
  // Enforced here and not only in the app: the client gate can be bypassed by
  // anyone calling the API directly, and this record is the evidence that the
  // partner agreed to the commission terms and the liability clauses.
  if(terms_accepted!==true||!terms_version)
    return res.status(400).json({ success:false, message:'You must accept the Partner Terms to register' });
  ...
  try {
    ...
    const lm = await client.query(
      "INSERT INTO laundromats(name,owner_name,email,phone,address,area,city,mpesa_till,description,commission_rate,admin_fee_rate,status,payout_method,payout_number,payout_account_ref,payout_name,payout_verified,terms_accepted_at,terms_version,terms_accepted_ip) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,15,5,'pending',$10,$11,$12,$13,false,NOW(),$14,$15) RETURNING *",
      [name, owner_name, nEmail, np, address, area||null, city||'Nairobi', mpesa_till||null, description||null,
       ...
       (req.headers['x-forwarded-for']||'').split(',')[0].trim().slice(0,60)||req.ip]
    );
